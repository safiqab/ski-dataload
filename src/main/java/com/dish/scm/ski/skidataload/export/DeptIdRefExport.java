...
@Component
public class DeptIdRefExport {
public void writeData(Sheet sheet,List<DeptIdRefVM> lstData) {
		...
		try {
			...
			for (DeptIdRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getDept_id());
				row.createCell(1).setCellValue(obj.getDept_name());
				row.createCell(2).setCellValue(obj.getDept_desc());
