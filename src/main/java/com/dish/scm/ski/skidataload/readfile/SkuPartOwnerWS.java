...
@Component
public class SkuPartOwnerWS {
    @Value("{skip.header:Y")
    private String skip_header;

    public List<SkuPartOwnerVM> readSheet(Sheet worksheet) {

        DataFormatter dataFormatter = new DataFormatter();
        List<SkuPartOwnerVM> lstData = new ArrayList<>();
        worksheet.iterator().forEachRemaining(x->
        {
            if (!isRowEmpty(x)) {

                SkuPartOwnerVM obj = new SkuPartOwnerVM();
                if (skip_header.equals("N")) {
                    ...
                    while (colIterator.hasNext()) {
                        ...
                        obj.setPart_owner(dataFormatter.formatCellValue(nextCol.getRow().getCell(2)).trim());
                        obj.setPart_number(dataFormatter.formatCellValue(nextCol.getRow().getCell(3)).trim());
