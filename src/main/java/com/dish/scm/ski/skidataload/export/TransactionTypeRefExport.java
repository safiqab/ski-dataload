...
@Component
public class TransactionTypeRefExport {
	
public void writeData(Sheet sheet,List<TransactionTypeRefVM> lstData) {
		...
		try {
			...
			for (TransactionTypeRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getTrans_type());
				row.createCell(1).setCellValue(obj.getTrans_name());
				row.createCell(2).setCellValue(obj.getTrans_desc());
