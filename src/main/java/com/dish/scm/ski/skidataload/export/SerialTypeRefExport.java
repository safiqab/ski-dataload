...
@Component
public class SerialTypeRefExport {
	
public void writeData(Sheet sheet,List<SerialTypeRefVM> lstData) {
		...
		try {
			...
			for (SerialTypeRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getSerial_type());
				row.createCell(1).setCellValue(obj.getSerial_type_name());
				row.createCell(2).setCellValue(obj.getSerial_type_desc());
