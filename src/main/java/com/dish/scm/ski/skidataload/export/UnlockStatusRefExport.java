...
@Component
public class UnlockStatusRefExport {
	
public void writeData(Sheet sheet,List<UnlockStatusRefVM> lstData) {
		...
		try {
			...
			for (UnlockStatusRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getUnlock_status_code());
				row.createCell(1).setCellValue(obj.getUnlock_status_name());
				row.createCell(2).setCellValue(obj.getUnlock_status_desc());
