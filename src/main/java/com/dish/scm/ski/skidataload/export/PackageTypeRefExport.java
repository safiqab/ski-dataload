...
@Component
public class PackageTypeRefExport {
	
public void writeData(Sheet sheet,List<PackageTypeRefVM> lstData) {
		...
		try {
			...
			for (PackageTypeRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getPackage_type());
				row.createCell(1).setCellValue(obj.getPackage_type_name());
				row.createCell(2).setCellValue(obj.getPackage_type_desc());
