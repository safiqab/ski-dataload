...
@Component
public class SkuOwnerRefWS {
    @Value("{skip.header:Y")
    private String skip_header;

    public List<SkuOwnerRefVM> readSheet(Sheet worksheet) {

        DataFormatter dataFormatter = new DataFormatter();
        List<SkuOwnerRefVM> lstData = new ArrayList<>();
        worksheet.iterator().forEachRemaining(x->
        {
            if (!isRowEmpty(x)) {

                SkuOwnerRefVM obj = new SkuOwnerRefVM();
                if (skip_header.equals("N")) {
                    ...
                    while (colIterator.hasNext()) {
                        ...
                        obj.setSku_owner(dataFormatter.formatCellValue(nextCol.getRow().getCell(0)));
