...
@Component
public class SimTypeRefExport {
	
	public void writeData(Sheet sheet,List<SimTypeRefVM> lstData) {
		...
		try {
			...
			for (SimTypeRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getSim_type());
				row.createCell(1).setCellValue(obj.getSim_type_name());
				row.createCell(2).setCellValue(obj.getSim_type_desc());
