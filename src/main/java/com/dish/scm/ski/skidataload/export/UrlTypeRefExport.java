...
@Component
public class UrlTypeRefExport {
	
	public void writeData(Sheet sheet,List<UrlTypeRefVM> lstData) {
		...
		try {
			...
			for (UrlTypeRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getUrl_type());
				row.createCell(1).setCellValue(obj.getUrl_name());
				row.createCell(2).setCellValue(obj.getUrl_desc());
