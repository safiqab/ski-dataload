...
@Component
public class ShipmentStatusRefExport {
	
public void writeData(Sheet sheet,List<ShipmentStatusRefVM> lstData) {
		...
		try {
			...
			for (ShipmentStatusRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getShipment_status());
				row.createCell(1).setCellValue(obj.getShipment_status_desc());				
				row.createCell(2).setCellValue(obj.getError_status());
				row.createCell(3).setCellValue(replaceNull(obj.getError_message()));
			}
		}
		catch (Exception e) {
			e.printStackTrace();
