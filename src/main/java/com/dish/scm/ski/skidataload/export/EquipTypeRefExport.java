...
@Component
public class EquipTypeRefExport {
	
public void writeData(Sheet sheet,List<EquipTypeRefVM> lstData) {
		...
		try {
			...
			for (EquipTypeRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getEquip_type());
				row.createCell(1).setCellValue(obj.getEquip_type_name());
				row.createCell(2).setCellValue(obj.getEquip_type_desc());
