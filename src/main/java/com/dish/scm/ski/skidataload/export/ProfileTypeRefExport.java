...
@Component
public class ProfileTypeRefExport {
	
public void writeData(Sheet sheet,List<ProfileTypeRefVM> lstData) {
		...
		try {
			...
			for (ProfileTypeRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getProfile_type());
				row.createCell(1).setCellValue(obj.getProfile_type_name());
				row.createCell(2).setCellValue(obj.getProfile_type_desc());
