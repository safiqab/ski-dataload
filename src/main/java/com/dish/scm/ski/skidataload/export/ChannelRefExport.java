...
@Component
public class ChannelRefExport {

public void writeData(Sheet sheet,List<ChannelRefVM> lstData) {
		...
		try {
			...
			for (ChannelRefVM obj : lstData) {
				
				row = sheet.createRow(rowCnt++);
				row.createCell(0).setCellValue(obj.getChannel_code());
				row.createCell(1).setCellValue(obj.getChannel_name());
				row.createCell(2).setCellValue(obj.getChannel_desc());
