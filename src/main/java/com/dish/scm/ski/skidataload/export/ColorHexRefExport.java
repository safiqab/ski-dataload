...
@Component
public class ColorHexRefExport {
    public void writeData(Sheet sheet, List<ColorHexRefVM> lstData) {
        ...
        try {
            ...
            for (ColorHexRefVM obj : lstData) {

                row = sheet.createRow(rowCnt++);
                row.createCell(0).setCellValue(obj.getColor_hex());
                row.createCell(1).setCellValue(obj.getColor_hex_name());
                row.createCell(2).setCellValue(obj.getColor_hex_desc());
