...
@Component
public class ModelNameWorkflowWS {
    @Value("{skip.header:Y")
    private String skip_header;

    public List<ModelNumVM> readSheet(Sheet worksheet) {
            ...
            if (!isRowEmpty(x)) {
                ...
                if (skip_header.equals("N")) {
                    ...
                    while (colIterator.hasNext()) {
                        ...
                        obj.setModelNumber(dataFormatter.formatCellValue(nextCol.getRow().getCell(0)));
                        obj.setEquipType(dataFormatter.formatCellValue(nextCol.getRow().getCell(1)));
                        break;
                    }
                    if (hasData)
                        lstData.add(obj);
